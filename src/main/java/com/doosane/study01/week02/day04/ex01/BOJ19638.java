...
public class BOJ19638 {
    public static void main(String[] args) throws IOException {
        ...
        int tLimit = Integer.parseInt(firstLine[2]); //  망치 횟수 제한 -> 첫번째 줄, 세번째 공백 인덱스 2
        ...
        while (!pq.isEmpty() && pq.peek() >= hCenti && count < tLimit) {  // 우선순위큐가 빈 값이 아니고, pq.peek()으로 가져온값 , 제일 큰 값이 센티의 키보다 크거나 같은 조건에서
            int max = pq.poll(); // pq.poll() 제일 큰 거인의 키를 가져와서 max 에 할당
            if (max > 1) {
        ...
        if (pq.isEmpty() || pq.peek() < hCenti) {   // 큐가 비었거나 제일 큰 값을 가져왔을때 센티보다 작을 경우
            sb.append("YES\n").append(count);      //YES를 출력하고, 망치를 최소로 사용한 횟수를 출력

        } else {                                   // 센티보다 클 경우
            sb.append("NO\n").append(pq.peek());  // NO를 출력하고, 가장 키가 큰 거인의 키를 출력한다.
        }

        System.out.println(sb); // StringBuilder로 한번에 모았다가 출력
        br.close(); // 메모리 누수 방지를 위해 BufferedReader를 종료
    }
}
